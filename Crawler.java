...
public class Crawler {
    ...
    private static LinkedList<String> getAllLinks(URLDepth myDepthPair) {
        ...
        // Попытка создать сокет для доступа к указанному в паре URLDepth сайту через порт 80
        try {
            sock = new Socket(myDepthPair.getWebHost(), 80);
        }
        // Ловим и обрабатываем UnknownHostException, возвращается пустой список.
        catch (UnknownHostException e) {
            System.err.println("UnknownHostException: " + e.getMessage());
            return URLs;
        }
        // Ловим и обрабатываем IOException, возвращается пустой список
        catch (IOException ex) {
            System.err.println("IOException: " + ex.getMessage());
            return URLs;
        }
        
        // устанавливаем время отключения сокета на 3 секунды
        try {
            sock.setSoTimeout(3000);
        }
        // Ловим SocketException и возвращаем пустой список ссылок
        ...
        */
        ...
        // Объявляем поток выходных данных (от сервера)
        ...
        // Ловим IOException и возвращаем пустой список ссылок.
        catch (IOException exce) {
            System.err.println("IOException: " + exce.getMessage());
            return URLs;
        }
        
        //Создаем объект  PrintWriter. True означает, что PrintWriter будет очищаться после каждого запроса
